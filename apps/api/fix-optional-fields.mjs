...
async function fixOptionalFields() {
  ...
  try {
    ...
    const scriptPath = path.join(__dirname, 'fix-optional-fields.surql');
    const script = await fs.readFile(scriptPath, 'utf8');

    console.log("Ejecutando corrección de campos opcionales...");
    const result = await db.query(script);

    console.log("✓ Campos opcionales corregidos");
