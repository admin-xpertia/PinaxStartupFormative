...
import type { GlossaryEntry, LeccionContent, LessonVerificationQuestion } from "@/types/content"
  ...
  const handleCheckAnswer = (question: LessonVerificationQuestion) => {
    ...
    updateState(question.id, (prev) => ({
      ...
      feedback: isCorrect
        ...
        : question.feedback.porOpcion?.find((entry) => selected.includes(entry.optionId))?.feedback ||
        ...
        <div className="space-y-4">
          <div>
            ...
            {question.criteriosEvaluacion && question.criteriosEvaluacion.length > 0 && (
              <p className="mt-1 text-xs text-slate-500">{question.criteriosEvaluacion.join(" • ")}</p>
            )}
          </div>
    ...
    >
      ...
      {!hasInlineVerificationBlocks && content.preguntasVerificacion && content.preguntasVerificacion.length > 0 && (
        <section className="rounded-3xl border border-slate-200 bg-white/70 p-6">
          ...
          <p className="text-sm text-slate-500">Estas preguntas se agregan automáticamente cuando el markdown no las incluye embebidas.</p>
